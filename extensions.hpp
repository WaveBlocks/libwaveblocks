...
struct ExponentialImplemenation {
  ...
  evaluate_exponential(grid_in<RVector<D> > args, real_t factor = 1) {
    return evaluate_function_in_grid<RVector<D>, potential_evaluation_type,
                                     grid_in, grid_out, function_t>(
        std::bind(&self_type::evaluate_exponential_at, this,
                  std::placeholders::_1, factor),
        args);
  }
};

//~ template<template<int, int> class B1, template<int,int> class B2, int N, int
//D>
...
//~ }
//~ else {
//~ return evaluate_function_in_grid<RVector<D>, CMatrix<N,N>, Grid,
//function_t>(eigen_basis_transform,g);
//~ }
//~ }
//~ };
