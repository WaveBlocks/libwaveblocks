...
public:
	...
	MatrixToGridIterator(MatrixToGridIterator& adaptor, int i) : g(adaptor), i(i) {}
	bool operator!=(MatrixToGridIterator<Matrix> other) {return other.i != i;}
	
	void operator++() {++i;}
	const grid_element_type& operator*() {
		return g[i];
...
struct Helper {
	template<class I>
	using type = MatrixToGrid<GMatrix<typename I::Scalar,I::RowsAtCompileTime,M>>;
...
template<int N, template<typename, int> class V, class A, class R, template<typename ...> class G_in = std::vector, template<typename ...> class G_out = G_in, template<typename ...> class F = std::function>
...
template<int N, template<typename, int, int> class M, class A, class R, template<typename ...> class G_in = std::vector, template<typename ...> class G_out = G_in, template<typename ...> class F = std::function>
G_out<M<R,N,N>> evaluate_function_matrix_in_grid(M<F<R(A)>,N,N> mf, G_in<A> g)
{
