...
template <template <template <int, int> class, int, int> class S,
          template <int, int> class B, int N, int D>
struct EvaluationAbstract {
  ...
  Tuple<> taylor_at(RVector<D> g) {
    return Tuple<>(evaluate_at(g), evaluate_jacobian_at(g),
                   evaluate_hessian_at(g));
  }
...
struct EvaluationImplementation<
    S, CanonicalBasis, N, D> : EvaluationAbstract<S, CanonicalBasis, N, D> {
...
public:
  EvaluationImplementation(potential_type potential, jacobian_type jacobian,
                           hessian_type hessian)
...
public:
  ...
  evaluate_local_remainder_at_implementation(RVector<D> g,
                                             RVector<D> position) {
    return evaluate_function_matrix<N, GMatrix, RVector<D>,
                                    potential_return_type, function_t>(
        that.local_remainder(position), g);
  }
};

template <template <template <int, int> class, int, int> class S, int N, int D>
struct EvaluationImplementation<S, EigenBasis, N,
...
public:
  ...
  evaluate_local_remainder_at_implementation(RVector<D> g,
    ...
    return evaluate_function_vector<N, GVector, RVector<D>,
                                    potential_return_type, function_t>(
        this->local_remainder(position), g);
  }
};
