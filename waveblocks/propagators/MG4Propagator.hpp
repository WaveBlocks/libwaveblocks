...
/**
 * \brief implements the Magnus MG4 Propagator
 *
 * This class implements the (pre-/post-)propagate functions for the Magnus MG4 Propagator
 */
...
class MG4Propagator : public Propagator<MG4Propagator<N,D,MultiIndex_t,MDQR_t,Potential_t,Packet_t,Coef_t>,N,D,MultiIndex_t,MDQR_t,Potential_t,Packet_t,Coef_t> {

	public:

		// inherit constructor
		using Propagator<MG4Propagator<N,D,MultiIndex_t,MDQR_t,Potential_t,Packet_t,Coef_t>,N,D,MultiIndex_t,MDQR_t,Potential_t,Packet_t,Coef_t>::Propagator;

		std::string getName() { return "Magnus MG4"; }
		...
		void propagate(const real_t Dt) {
			...
			this->buildF(A1);
			A1 *= complex_t(0,-1./(packet.eps()*packet.eps()));
			this->intSplit(h2,M2,this->splitCoef_);
			this->buildF(A2);
			A2 *= complex_t(0,-1./(packet.eps()*packet.eps()));
			this->F_ = .5*Dt*(A1+A2) + std::sqrt(3)/12*Dt*Dt*(A2*A1-A1*A2);
