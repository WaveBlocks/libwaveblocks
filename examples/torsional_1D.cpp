...
int main() {
    ...
    typename CanonicalBasis<N,D>::potential_type potential = [](complex_t x) {
		return 1. - std::cos(x.real());
    };
    typename ScalarLeadingLevel<D>::potential_type leading_level = potential;
    typename ScalarLeadingLevel<D>::jacobian_type leading_jac = [](complex_t x) {
        return std::sin(x.real());
    };
    typename ScalarLeadingLevel<D>::hessian_type leading_hess = [](complex_t x) {
	...
	// Hagedorn
	{
    io::hdf5writer<D> writerHagedorn("torsional_1D_Hagedorn.hdf5"); writerHagedorn.set_write_norm(true); writerHagedorn.set_write_energies(true);
	...
	// Semiclassical
	{
    io::hdf5writer<D> writerSemiclassical("torsional_1D_Semiclassical.hdf5"); writerSemiclassical.set_write_norm(true); writerSemiclassical.set_write_energies(true);
	...
	// MG4
	{
    io::hdf5writer<D> writerMG4("torsional_1D_MG4.hdf5"); writerMG4.set_write_norm(true); writerMG4.set_write_energies(true);
	...
	// McL42
	{
    io::hdf5writer<D> writerMcL42("torsional_1D_McL42.hdf5"); writerMcL42.set_write_norm(true); writerMcL42.set_write_energies(true);
	...
	// McL84
	{
    io::hdf5writer<D> writerMcL84("torsional_1D_McL84.hdf5"); writerMcL84.set_write_norm(true); writerMcL84.set_write_energies(true);
	...
	// Pre764
	{
    io::hdf5writer<D> writerPre764("torsional_1D_Pre764.hdf5"); writerPre764.set_write_norm(true); writerPre764.set_write_energies(true);
