...
Thirdly, due to how lexical order works, the distance between \((\mindex{k}-\mindex{e}^{d-1})\) and \((\mindex{k}-\mindex{e}^d)\)
...
\begin{algorithm}[H]
  ...
  \caption{Computes coefficients of the wavepacket gradient \(\nabla \Phi\)}
\end{algorithm}
